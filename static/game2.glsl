...
void main() {
  ...
  float d = 4.0;
  ro = vec3(d * cos(mouse.x), 0.1+d * mouse.y, d * sin(mouse.x)) + playerpos[playerid].xyz;
...
vec2 raymarch(vec3 ro, vec3 rd) {
  ...
  float ground = -ro.y / rd.y; //iPlane simplifies to this if plane=vec4(0.0, 1.0, 0.0, 0.0)
  if (ground > 0.0) {
    res = vec2(ground, -1.0);
  }
...
// return distance and which model number was hit by ray (without marching aka raytrace)
...
vec3 skycolor(vec3 rd) {
  return vec3(0.7, 0.7, 0.9) - max(rd.y, 0.0) * 0.3;
}
...
vec3 groundcolor(vec3 rd, vec3 p) {
  ...
  vec3 Lo = CookTorranceBRDF(N, V, p, lightpos, lightcolor, lightstrength, F0, albedo, roughness, metallic);
...
vec2 spherenormal2uv(vec3 N) {
  ...
  float v = 0.5*N.y + 0.5;
  return vec2(u,v);
  //return vec2(u*0.75,v*0.75); //texture is a bit too fine grained. make features "larger"
}
